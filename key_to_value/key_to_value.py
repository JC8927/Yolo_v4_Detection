...
def find_drawing_spot(bounding_poly):
    min_x=None
    min_y=None
    max_x=None
    max_y=None
...
def draw_bounding_box(img,bounding_poly,color):

    min_x,max_x,min_y,max_y = find_drawing_spot(bounding_poly)
    idx_img = img.copy()
    cv2.rectangle(idx_img, (min_x, min_y), (max_x, max_y), color,thickness=3)
    saved_idx_img = idx_img.copy()
...
#         min_x,max_x,min_y,max_y = find_drawing_spot(ocr_bounding_poly)
...
def data_preprocess(results,record_list,record_dict)-> List[dict]:#傳入圖片ocr辨識結果  
    imformation_list=[]
    ...
    for j in range(len(results)):
        ...
        width = int(location[1][0]-location[0][0])
        height = int(location[2][1]-location[1][1])
        ...
        if whole_text.find(' ') != -1:#可能要再改
                    ...
                    for col_name in col_list:
                        ...
                        if col_name.find(' ') != -1:
                            ...
                            for k,text in enumerate(col_text_list):
                                ...
                                if cur_col_text ==  cur_text:
                                    ...
                                    if k == 0:
                                        first_flag = True
                                    match_flag = True
                                    diction={'cur_text':cur_col_text,'width':width,'height':height,'after_text':after_text,'col_list_name':col_list_name,'col_name':col_name,'text_order':i}
                            ...
                            if col_name == cur_text:
                                ...
                                match_flag = True
                                first_flag = True
                                diction={'cur_text':cur_text,'width':width,'height':height,'after_text':after_text,'col_list_name':col_list_name,'col_name':col_name,'text_order':i}
                                match_col_name_list.append(diction)

                if first_flag == False or first_flag ==True:
                    diction={'x':location_mid_x,'y':location_mid_y,'width':width,'height':height,'text':cur_text,'bounding_poly':location,'score':score,'col_name_flag':False}
                ...
                for diction in match_col_name_list:
                    ...
                    if after_text != None and now_col_name.find(' ') != -1:
                        ...
                        if combined_text == now_col_name:
                            diction={'x':location_mid_x,'y':location_mid_y,'width':width,'height':height,'text':combined_text,'bounding_poly':location,'score':score,'col_name_flag':True,'start_order':start_order,'end_order':end_order}
                            temp_imformation_list.append(diction)
                    else:
                        if after_text != None and now_col_name == cur_text:
                            combined_text = cur_text+" "+after_text
                            diction={'x':location_mid_x,'y':location_mid_y,'width':width,'height':height,'text':cur_text,'bounding_poly':location,'score':score,'col_name_flag':True,'start_order':start_order,'end_order':end_order}
                            temp_imformation_list.append(diction)
            ...
            for col_list_name in record_dict.keys():
                ...
                for col_name in col_list:
                    if col_name == cur_text :
                        match_flag = True
                        diction={'x':location_mid_x,'y':location_mid_y,'width':width,'height':height,'text':cur_text,'bounding_poly':location,'score':score,'col_name_flag':True}
                        imformation_list.append(diction)
            if match_flag == False:
                diction={'x':location_mid_x,'y':location_mid_y,'width':width,'height':height,'text':cur_text,'bounding_poly':location,'score':score,'col_name_flag':False}
                imformation_list.append(diction)
...
#比對barcode與ocr結果
def compare_barcode_data(img,col_dict,barcode_list):
    col_data = col_dict['col_data']
    col_data_bounding_poly = col_dict['bounding_poly']

    #找出ocr_data座標
    col_data_x = (col_data_bounding_poly[0][0]+col_data_bounding_poly[1][0])/2
    col_data_y = (col_data_bounding_poly[1][1]+col_data_bounding_poly[2][1])/2

    for barcode in barcode_list:
        barcode_direction = ""
        barcode_text = barcode['text']
        barcode_bounding_poly = barcode['bounding_poly']
        barcode_x = barcode['x']
        barcode_y = barcode['y']
        score = fuzz.ratio(col_data,barcode_text)
        if score>70:
            diff_x = int(col_data_x-barcode_x)
            diff_y = int(col_data_y-barcode_y)
            if (diff_x>diff_y): #在左右
                if(diff_x>0):   #barcode在data左邊
                    barcode_direction="left"
                elif(diff_x<0): #barcode在data右邊
                    barcode_direction="right"
            elif(diff_y>=diff_x):
                if(diff_y>0):
                    barcode_direction="up" #barcode在data上面
                elif(diff_y<0):
                    barcode_direction="down" #barcode在data下面
            color=(255,0,0)
            idx_img = draw_bounding_box(img,barcode_bounding_poly,color)
            print("請問此barcode是否對應到"+str(col_data)+"? 是請輸入:T")
            barcode_flag = str(input())
            if barcode_flag.upper()=="T":
                cv2.destroyAllWindows()
                result_diction={"ocr_text":col_data,"barcode_text":barcode_text,"barcode_direction":barcode_direction,"barcode_diff_x_ratio":0,"barcode_diff_y_ratio":0}
                return result_diction
    
    result_diction={"ocr_text":col_data,"barcode_text":"no_barcode_result","barcode_direction":-1,"barcode_diff_x_ratio":0,"barcode_diff_y_ratio":0}
    return result_diction
        

def compare_col_data(barcode_list,imformation_list,col_name,config_list,config,col_data_idx_list,idx,idx_col_data,col_name_x,col_name_y,pass_num,img):
    col_data_list=[]
    idx_col_data=[idx_col_data]
        ...
        if detect_mode =="0":
            for data_idx in col_data_idx_list:
                if data_idx+idx<len(imformation_list) and data_idx+idx>-1:
                    diff_y_threshold = imformation_list[idx]['height']
                    col_data = imformation_list[idx+data_idx]['text']
                    now_bounding_poly = imformation_list[idx+data_idx]['bounding_poly']
                    ...
                    if diff_x >=0 and diff_y<=diff_y_threshold:
                        diction={'col_data':col_data,'diff_y':abs(diff_y),'diff_x':diff_x,'bounding_poly':now_bounding_poly,'idx':idx+data_idx,'col_name_flag':col_name_flag}
                        col_data_list.append(diction)
                else:
                    continue
        #上下模式
        elif detect_mode == "1":
            imformation_list_1 = imformation_list.copy()
            imformation_list_1 = sorted(imformation_list_1,key=lambda d:d['x'])
            for i,imformation in enumerate(imformation_list_1) :
                if imformation == imformation_list[idx]:
                    idx = i
                    break
            for data_idx in col_data_idx_list:
                if data_idx+idx<len(imformation_list_1):
                    diff_x_threshold = imformation_list_1[idx]['width']
                    col_data = imformation_list_1[idx+data_idx]['text']
                    now_bounding_poly = imformation_list_1[idx+data_idx]['bounding_poly']
                    diff_y = imformation_list_1[idx+data_idx]['y']-imformation_list_1[idx]['y']
                    diff_x = imformation_list_1[idx+data_idx]['x']-imformation_list_1[idx]['x']
                    col_name_flag = imformation_list_1[idx+data_idx]['col_name_flag']
                    if diff_y>=0 and diff_x<=diff_x_threshold:
                        diction={'col_data':col_data,'diff_y':diff_y,'diff_x':abs(diff_x),'bounding_poly':now_bounding_poly,'idx':idx+data_idx,'col_name_flag':col_name_flag}
                        col_data_list.append(diction)
                else:
                    continue
    else:
        ...
        if mode_check == "1":
            ...
            for i,imformation in enumerate(imformation_list_1) :
                if imformation == imformation_list[idx]:
                    ...
                    break
            for data_idx in col_data_idx_list:
                if data_idx+idx<len(imformation_list_1) and data_idx+idx>-1:
                    diff_x_threshold = imformation_list_1[idx]['width']
                    col_data = imformation_list_1[idx+data_idx]['text']
                    now_bounding_poly = imformation_list_1[idx+data_idx]['bounding_poly']
                    diff_y = imformation_list_1[idx+data_idx]['y']-imformation_list_1[idx]['y']
    ...
    for i,col_dict in enumerate(col_data_list):
        ...
        bounding_poly = col_dict['bounding_poly']
        data = col_name+":"+col_data
        if config == None:
            if col_name_flag == True:
                continue
            color=(0,255,0)
            idx_img = draw_bounding_box(img,bounding_poly,color)
            ...
            if check.upper()=="T":
                ...
                if text_type=='str':
                    if col_data.isdigit():
                        text_type='int'
                text_type_dict={"LOT":"str",'PN':'str','COO':'str','DATE':'str','SN':'str'}
                for name in text_type_dict.keys():
                    if name == col_name:
                        text_type = text_type_dict[name]
                text_type_list.append(text_type)
                text_len_list.append(text_len)
                recorded_data_list.append(combined_result_diction)
                recorded_data_idx_list.append(col_data_idx)
                text_num = text_num+1
                col_data_correct_flag=True
                barcode_direction = combined_result_diction['barcode_direction']
                reference_barcode_text = combined_result_diction['barcode_text']
            ...
            if text_type=='str':
                if col_data.isdigit():
                    text_type='int'
            record_list=['QTY','LOT','DATE','PN','COO','SN']
            text_type_dict={"LOT":"str",'PN':'str','COO':'str','DATE':'str','SN':'str'}
            for name in text_type_dict.keys():
                if name == col_name:
                    text_type = text_type_dict[name]
            ...
            for j,config_text_type in enumerate(config_text_type_list):
                if text_type == config_text_type:
                    ...
                    if col_name == 'COO':#國家
                        ...
                        country_list=['TW','CN','CHINA','MALAYSIA','TH','THAILAND','TAIWAN']
                        for country in country_list:
                            if country.upper() == col_data.upper():
                                col_data_correct_flag = True
                                detected_flag = True
...
def first_compare(barcode_list,imformation_list,config_path,image_path,record_list,record_dict,result_dict,result_list)-> Union[List[dict],List[dict]]:
            ...
            for col_name in col_list:
                ...
                if score>max_score:
                    ...
                    if max_score==100:
                        ...
                        #result_dict[col_list_name].append(i)
                        break                  
            if max_score>80 and record_flag == False: #如果分數達標 認定為col
                max_score_diction={'max_score':max_score,'col':col_list_name}#紀錄max_score分數
                max_score_list.append(max_score_diction)
                #result_dict[col_list_name].append(i)
        if len(max_score_list)!=0:
            for max_score_dict in max_score_list:
                col_list_name=max_score_dict['col']
    ...
    for col_name in result_dict.keys():
        ...
        for idx in result_dict[col_name]:
                ...
                if diction['bounding_poly'] == bounding_poly:
                    ...
                    for key in keys:
                        if col_name == key:
                            #認定存在
                            for saved_config in config_list:
                                if saved_config['col_name_x'] == idx_col_x and saved_config['col_name_y'] == idx_col_y and saved_config['col'] ==key:
                                    saved_config['col_name_text'].append(idx_col_data)
                            jump_flag = True
                            break
                    if jump_flag :
                        break
            ...
            cv2.waitKey(1)
            print("請問是否偵測到"+str(col_name)+"之欄位? ("+str(col_name)+":"+str(idx_col_data)+")")
            ...
            #找到tag_data
            col_data,recorded_col_data_idx_list=compare_col_data(barcode_list,imformation_list,col_name,config_list,config,col_data_idx_list,idx,idx_col_data,idx_col_x,idx_col_y,pass_num,idx_img)
            #比對tag_data及barcode
            if col_data == None:
                continue
            for i in range(len(col_data)):
                if label_num<i:
                    label_num = i
                now_col_data_diction = col_data[i]
                now_col_data = now_col_data_diction['ocr_text']
                now_col_data_idx = recorded_col_data_idx_list[i]
                now_barcode_text = now_col_data_diction['barcode_text']
                print("欄位資料(OCR):"+col_name+":"+now_col_data)
                print("欄位資料(barcode):"+col_name+":"+now_col_data)
                print("欄位idx:"+str(now_col_data_idx+idx))
                diction={'col_name':col_name,col_name:now_col_data,"barcode_text":now_barcode_text,"location":now_col_data_idx,"bounding_poly":bounding_poly,'label_id':0,'col_id':i}
                result_list.append(diction)
    ...
    for col_list_name in record_list:
        ...
        if exist_flag == False:
            ...
            check=input()
            if check.upper() =="T":
                print("請選擇擷取模式 1.取自其他tag data 2.使用data長度 3.特殊模式")
                mode_check = str(input())
                if mode_check == "1":
                    for result in result_list:
                        ...
                        if truncation_flag.upper()=="T":
                            print("請輸入愈擷取初始位置 EX:0")
                            trun_start = int(input())-1
                            print("請輸入愈擷取最終位置 EX:4")
                            trun_end = int(input())
                            now_text = now_col_data[trun_start:trun_end]
                            text_length=len(now_text)
                            result_diction={'col_name':col_list_name,col_list_name:now_text,"location":0,"bounding_poly":now_bounding_poly,'label_id':0,'col_id':0}
                            no_col_config_diction={'mode':mode_check,'take_from_tag':now_col_name,'start':trun_start,'end':trun_end,'col':col_list_name,col_list_name:now_text,'text_length':text_length,'text_type':'str','col_name_idx':-1,'col_data_idx':col_data_idx_list[i],'col_data_idx_list':col_data_idx_list,'col_name_text':'','col_data_text':now_text}
                            result_list.append(result_diction)
                            no_col_config_list.append(no_col_config_diction)
                    ...
                    for imformation in imformation_list:
                        ...
                        if text_length == now_text_length:
                            ...
                            if confirm.upper() == "T":
                                result_diction={'col_name':col_list_name,col_list_name:now_text,"location":0,"bounding_poly":now_bounding_poly,'label_id':0,'col_id':0}
                                no_col_config_diction={'mode':mode_check,'col':col_list_name,col_list_name:now_text,'text_length':text_length,'text_type':'str','col_name_idx':-1,'col_data_idx':col_data_idx_list[i],'col_data_idx_list':col_data_idx_list,'col_name_text':'','col_data_text':now_text}
                                result_list.append(result_diction)
                                no_col_config_list.append(no_col_config_diction)
                                break
                elif mode_check =="3":
                    ...
                    saved_imformation_list = sorted(saved_imformation_list,key=lambda d:d['height'],reverse=True)
                    pre_bounding_poly = None
                    for imformation in result_list:
                        bounding_poly = imformation['bounding_poly']
                        if bounding_poly == pre_bounding_poly:
                            continue
                        min_x,max_x,min_y,max_y = find_drawing_spot(bounding_poly)
                        idx_img=img.copy()
                        cv2.rectangle(idx_img, (min_x, min_y), (max_x, max_y), (0,0,255),thickness=3)
                        ...
                        cv2.imshow("show_col",resize_idx_img)
                        cv2.waitKey(1)
                        print("請問"+imformation['col_name']+"是否為特徵點? 是請輸入:T")
                        spec_flag = str(input())
                        if "T" == spec_flag.upper():
                            jump_flag =True
                            diff_y_threshold = max_y-min_y
                            spec_x = int((min_x+max_x)/2)
                            spec_y = int((min_y+max_y)/2)
                            data_num = 0
                            print("請輸入紀錄之範圍倍率")
                            para = int(input())
                            print("請輸入比對方向 0:上 1:下 2:左 3:右")
                            direction = int(input())
                            record_flag = False
                            if direction ==2:
                                for now_imformation in imformation_list:
                                    now_x = now_imformation['x']
                                    now_y = now_imformation['y']
                                    now_text = now_imformation['text']
                                    now_bounding_poly = now_imformation['bounding_poly']
                                    idx_img=img.copy()
                                    ...
                                    cv2.imshow("show_col",resize_idx_img)
                                    cv2.waitKey(1)
                                    diff_y = abs(spec_y-now_y)
                                    if(now_x<=spec_x and diff_y<para*diff_y_threshold):
                                        print("請問"+now_text+"是否為愈紀錄之data? 是請輸入:T 紀錄完畢請按Q")
                                        ans_check = str(input())
                                        if "T" == ans_check.upper():
                                            record_flag= True
                                            result_diction={'col_name':col_list_name,col_list_name:now_text,"location":0,"bounding_poly":now_bounding_poly,'label_id':0,'col_id':data_num}
                                            data_num = data_num+1
                                            record_data_list.append(now_text)
                                            result_list.append(result_diction)

                                        elif "Q" == ans_check.upper():
                                            no_col_config_diction={'mode':mode_check,'col':col_list_name,col_list_name:record_data_list}
                                            break
                                        else:
                                            continue
                                if record_flag == True:
                                    no_col_config_diction={'mode':mode_check,'direction':direction,'ratio':para,'spec_col':imformation['col_name'],'col':col_list_name,col_list_name:record_data_list}
                                    no_col_config_list.append(no_col_config_diction)
                            elif direction == 3:
                                for now_imformation in imformation_list:
                                    now_x = now_imformation['x']
                            ...
                            pre_bounding_poly = bounding_poly
...
def normal_compare(barcode_list,imformation_list,config,config_2,image_path,record_list,record_dict,result_dict,result_list)-> Union[List[dict],List[dict]]:

    img=cv2.imread(image_path)
    config_list=config
    #確認目前col_name是否正確
    def check_col_name(now_col_name,now_idx,record_dict,record_col_name_list,imformation_list,pass_num) -> bool:
        ...
        forward_num = -1
        num = 0
        for record_col_name in record_col_name_list:
    ...
    y_imformation_list = sorted(y_imformation_list,key=lambda d:d['y'])
    label_id=-1
    col_id=-1
    x_config_list=[]
    y_config_list=[]
    ...
    for saved_config in config_list:
        if saved_config['detect_mode'] == "0":
            y_config_list.append(saved_config)
        elif saved_config['detect_mode'] == "1" :
    ...
    while len(y_imformation_list) != 0 and len(y_config_list)!=0:
        ...
        for saved_config in y_config_list:
            ...
            record_col_name_list = saved_config['col_name_text']
            record_col_name = record_col_name_list[0]
            ...
            if col_name_flag == False:
                if len(y_imformation_list) <= now_idx and max_idx != -1:
                    now_idx = max_idx
                
                col_name_flag,now_idx = check_col_name(now_col_name,now_idx,record_dict,record_col_name_list,y_imformation_list,pass_num)
            ...
            recorded_col_data_list,recorded_col_data_idx_list = compare_col_data(barcode_list,y_imformation_list,now_col_name,config_list,pre_saved_config,now_data_idx_list,now_idx,record_col_name,record_col_name_x,record_col_name_y,pass_num,img)
    ...
    while len(x_imformation_list) != 0 and len(x_config_list)!=0:
        ...
        for saved_config in x_config_list:
            ...
            pass_num = saved_config['pass_num']
            if col_name_flag == False:
                col_name_flag,now_idx = check_col_name(now_col_name,now_idx,record_dict,record_col_name_list,x_imformation_list,pass_num)
            ...
            pre_saved_config = copy.deepcopy(saved_config)
            recorded_col_data_list,recorded_col_data_idx_list = compare_col_data(x_imformation_list,now_col_name,config_list,pre_saved_config,now_data_idx_list,now_idx,record_col_name,record_col_name_x,record_col_name_y,pass_num,img)
            if recorded_col_data_list==None:
                continue
            for i in range(len(recorded_col_data_list)):
                col_data = recorded_col_data_list[i]
    ...
    if config_2!=None:
        for no_col_config in config_2:
            max_score = 0
            ans_text_list=[]
            now_col_name = no_col_config['col']
            config_mode = no_col_config['mode']
            possible_ans=[]
            ignore_flag = False
            if config_mode == "1":
                config_text = no_col_config['col_data_text']
                config_text_length = no_col_config['text_length']
                truncation_col = no_col_config['take_from_tag']
                trun_start = no_col_config['start']
                trun_end = no_col_config['end']
                col_name = no_col_config['col']
                for result in result_list:
                    result_col_name = result['col_name']
                    result_text = result[result_col_name]
                    bounding_poly = result['bounding_poly']
                    text_y = result['location']
                    if result_col_name == truncation_col:
                        now_text = result_text[trun_start:trun_end]
                        diction={'col_name':col_name,col_name:now_text,'text':now_text,'score':100,'location':text_y,'bounding_poly':bounding_poly,'col_id':0,'label_id':0}
                        result_list.append(diction)
                ...
                for imformation in imformation_list:
                    ignore_flag = False
                    imformation_bounding_poly = imformation['bounding_poly']
                    imformation_text = imformation['text']
                    ...
                    if imformation_text_length == config_text_length:
                        for result in result_list:
                            ...
                            if col_text == imformation_text and col_text.isalnum()==False:
                                ignore_flag = True
                                break
                        if ignore_flag == False:
                            diction={'col_name':now_col_name,'text':imformation_text,'location':imformation_text_y,'bounding_poly':imformation_bounding_poly}
                            possible_ans.append(diction)

            elif config_mode == "3":
                direction = no_col_config['direction']
                para = no_col_config['ratio']
                spec_col = no_col_config['spec_col']
                record_data_list=[]
                data_num=0
                for result in result_list:
                    bounding_poly = result['bounding_poly']
                    ...
                    spec_x = int((min_x+max_x)/2)
                    spec_y = int((min_y+max_y)/2)
                    if result['col_name'] == spec_col:
                        print("開始記錄"+now_col_name)
                        for now_imformation in imformation_list:
                            now_x = now_imformation['x']
                            now_y = now_imformation['y']
                            now_text = now_imformation['text']
                            ...
                            if(now_x<=spec_x and diff_y<para*diff_y_threshold):
                                ...
                                if "T" == ans_check.upper():
                                    record_flag= True
                                    result_diction={'col_name':now_col_name,now_col_name:now_text,"location":0,"bounding_poly":now_bounding_poly,'label_id':0,'col_id':data_num}
                                    data_num = data_num+1
                                    record_data_list.append(now_text)
                                    result_list.append(result_diction)
        ...
        if len(possible_ans)!=0:
            ...
            for text_diction in possible_ans:
                now_col_name = text_diction['col_name']
                text = text_diction['text']
                text_y = text_diction['location']
                bounding_poly = text_diction['bounding_poly']
                score = fuzz.ratio(text,config_text)
                diction={'col_name':now_col_name,now_col_name:text,'text':text,'score':score,'location':text_y,'bounding_poly':bounding_poly,'col_id':0}
                ans_text_list.append(diction)
            ans_text_list=sorted(ans_text_list,key=lambda d:d['score'])
            ans_text_list = ans_text_list[(len(ans_text_list)-max_label_id):]
            ans_text_list = sorted(ans_text_list,key=lambda d:d['location'])
            for i in range(max_label_id):
                ans_text = ans_text_list[i]
                ans_text['label_id'] = i
                result_list.append(ans_text)
...
# barcode找不到未設計完成
def barcode_compare_ocr(result_list,dbr_decode_res,record_list):#要改
            ...
            if col_name in result:
                ...
                if len(match_text_list) != 0:
                    ...
                    for diction in match_text_list:
                        ...
                        if diction['match_score']>max_score and diction['match_score']>80:
