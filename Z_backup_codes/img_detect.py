...
class Yolo_v4():
    def __init__(self,model_path,GPU_ratio=0.2):
        ...
        height =416 #416, 608
        width = 416#416, 608
        score_thresh = 0.2#0.5
        iou_thresh = 0.113#0.213
        max_box = 50
...
def real_time_obj_detection(model_path,GPU_ratio=0.2):
    ...
    while (cap.isOpened()):
        ...
        ret_1, img = cap.read()
        # ----get image
        ret, yolo_img = cap.read()

        if ret is True:
            #----YOLO v4 detection
            yolo_img = yolo_v4.detection(yolo_img)
            ...
            cv2.putText(yolo_img, FPS, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 21, 0), 3) #123, 255, 18
    ...
    cv2.imshow("YOLO v4 by JohnnyAI", yolo_img)
    # 儲存照片
    cv2.imwrite('./src/result_pic.jpg', img)
    input("Please press the Enter key to proceed")
    # 套用tesseract
    def text_reconition(text_string):
        pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
        print(f"\"{text_string}\"辨識結果:")
        print(pytesseract.image_to_string(img, lang="chi_tra+eng"))
        if len(pytesseract.image_to_string(img, lang="eng")) == 0:
            print("recognition failed ...")

    text_reconition('./src/result_pic.jpg')
    #####################################################
    # ----release
    yolo_v4.sess.close()
    cap.release()
...
if __name__ == "__main__":
    model_path = r"C:\Users\shiii\YOLO_v4-master\yolov4-QB_test_6_416.ckpt.meta"
    GPU_ratio = 0.7
